...
GraphFactory::GraphFactory(bool loadSudokuDatabase)
{
	...
	while(in.good())
	{
		...
		if(fileLinecount % 11 == 0)
		{
			...
			// (-2 because the next line is blank, and the one following is a # followed by difficulty)
			sudokuLinecount = -2;
		}
...
{
	(*difficulty) = _sudokuList[listIdx].second;
	// For a 9x9 grid
	int boxSize = 3;
